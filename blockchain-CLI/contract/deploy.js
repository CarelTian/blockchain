...
// 读取 ABI 和字节码
const abi = JSON.parse(fs.readFileSync(path.resolve(__dirname, './build/transaction.abi'), 'utf8'));
const bytecode = fs.readFileSync(path.resolve(__dirname, './build/transaction.bin'), 'utf8');

const abi2 = JSON.parse(fs.readFileSync(path.resolve(__dirname, './build/IPcore.abi'), 'utf8'));
const bytecode2 = fs.readFileSync(path.resolve(__dirname, './build/IPcore.bin'), 'utf8');
...
const deploy1 = async () => {
    ...
    console.log('Tx Contract deployed at address:', receipt.contractAddress);
};

const deploy2 = async () => {
    // 创建合约实例
    const contract = new web3.eth.Contract(abi2);

    // 构建部署交易
    const deployTx = contract.deploy({ data: bytecode2 });

    // 获取 Gas 估算
    const gas = await deployTx.estimateGas()+BigInt(300000);
    const latestBlock = await web3.eth.getBlock('latest');
    const baseFeePerGas = new BN(latestBlock.baseFeePerGas)
    const maxPriorityFeePerGas = new BN(web3.utils.toWei('50', 'gwei'));
    const maxFeePerGas =baseFeePerGas.add(maxPriorityFeePerGas);
